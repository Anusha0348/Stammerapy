...
if(isset($_POST['update'])){
   ...
   $update_exercise = $conn->prepare("UPDATE `exercise` SET title = ?, description = ?, status = ? WHERE id = ?");
   $update_exercise->execute([$title, $description, $status, $video_id]);

   if(!empty($playlist)){
      $update_playlist = $conn->prepare("UPDATE `exercise` SET playlist_id = ? WHERE id = ?");
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         $update_thumb = $conn->prepare("UPDATE `exercise` SET thumb = ? WHERE id = ?");
   ...
   if(!empty($video)){
      $update_video = $conn->prepare("UPDATE `exercise` SET video = ? WHERE id = ?");
...
if(isset($_POST['delete_video'])){
   ...
   $delete_video = $conn->prepare("SELECT video FROM `exercise` WHERE id = ? LIMIT 1");
...
<head>
   ...
   <title>Update exercise video</title>
...
<section class="video-form">
   ...
   <h1 class="heading">update exercise videos</h1>

   <?php
      $select_videos = $conn->prepare("SELECT * FROM `exercise` WHERE id = ? AND tutor_id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="hidden" name="old_video" value="<?= $fecth_videos['exercises']; ?>">
      ...
      <div class="flex-btn">
         <a href="view_exercise.php?get_id=<?= $video_id; ?>" class="option-btn">view exercise video</a>
         <input type="submit" value="delete video" name="delete_video" class="delete-btn">
      </div>
   </form>
   <?php
         }
      }else{
         echo '<p class="empty">video not found! <a href="add_exercise.php" class="btn" style="margin-top: 1.5rem;">add videos</a></p>';
      }
   ?>
